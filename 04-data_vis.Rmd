...
use_pkgs <- c("dplyr", "tidyr", "readr", "ggplot2", "cowplot", "lubridate", "scico")
...
Find a complete reference to *ggplot2* [here](https://ggplot2.tidyverse.org/) [@R-ggplot2]. The grammar of graphics has found its way also into Python and you can use ggplot using the plotnine Python package (see [here](https://realpython.com/ggplot-python/)).
...
```{r}
ddf <- read_csv("data/FLX_CH-Lae_FLUXNET2015_FULLSET_DD_2004-2006_DERIVED.csv")
...
Third, we chose a color palette that reflects the cyclic (or periodic) nature of the categories (months). January is closer to December than it is to April. Therefore, their respective colors should also be closer in *color space*. An appropriate palette for this is `"romaO"` from the *scico* package.
...
df_volcano <- tibble(
  ...
  height = c(volcano) - 140  # subtract 150 for example below
)

ggplot(df_volcano, aes(x = x, y = y, fill = height)) + 
  geom_raster() + 
  scale_fill_scico(palette = 'bukavu', midpoint = 0) + 
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))
```
...
Raster data often reflects values in geographic space. Also points and polygons may be located in geographic space. This opens the door to geospatial data visualisation (a.k.a. creating maps) and will be dealt with in more detail in Chapter \@ref(data_variety).

## Exercises

-   Using the data frame `ddf_allsites_nested_joined` from Chapter \@ref(data_wrangling), visualise the mean seasonal cycle of GPP for each site.
...
-   \*\*\*Tell a story about the `airquality` data.
