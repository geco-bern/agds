...
-   Timo Grossenbacher's [blog](https://timogrossenbacher.ch/blog/) on geographic data journalism
-   Koen Hufken's [blog](https://khufkens.com/blog/) containing various beautiful and interesting examples, always accompanied by a GitHub repo to reproduce them.

## Exercises

### Identifying Outliers {-}

A key part of data cleaning is to detect and understand where outliers are coming from. Visualisations make this task often informative. Your task here is to find outliers in `PPFD_IN`

-   Based on this and previous tutorials, read the half-hourly dataset for site CH-Lae, labels entries with `-9999` as `NA` and aggregate `GPP_NT_VUT_REF` and `PPFD_IN` to daily means. 
-   Fit a linear relationship between `GPP_NT_VUT_REF` and `PPFD_IN`
-   The function for fitting a linear model returns a list of objects which you can access using the `$` syntax.
    -   Access the fitted `residuals` and use them as argument in `boxplot.stats()` to quickly get box plot statistics that are useful to determine outliers.
    -   The output of `boxplot.stats()` is a list with the object `out`, a named vector where the names are the row numbers of the `residuals` in your original dataframe. The values are the entries for that row. Use `out_boxplot$out |> names() |> as.integer()` to get all row numbers.
-   Use these row numbers to create a new variable in the daily-aggregated dataset.
-   Create a plot that visualises the relationship between `GPP_NT_VUT_REF` and `PPFD_IN` and shows whether any given datapoint is an outlier or not.

### Visualising diurnal and seasonal cycles of GPP {-}

As explored in the previous Chapter's exercises, GPP varies strongly with the diurnal and seasonal cycle. Create a publication-ready figure showing both cycles in one single plot that is fully labelled, easy to read, and colorblind-friendly:

-   Again, start with the cleand the half-hourly dataset for site CH-Lae.
-   To get the diurnal and seasonal trend, summarise the half-hourly data by the hour of the day and the day of the year simultaneously and calculate mean values for GPP for each group.
-   Create a plot with {ggplot2} using the temporal variables as axes and GPP as color.
-   Make this figure ready for publication: 
    -   Add a title, maybe a subtitle
    -   Label axes and legend (don't forget to add units!)
    -   Pick a colorblind-friendly color scale

> Hint: The plot you are looking was introduced in the tutorial.

### Trend in carbon dioxide concentrations {-}

This exercise explores the change of CO$_2$, measured at the famous Mauna Loa observatory. Your task is to calculate and visualise the trend of CO$_2$ from 1958 to 2012, following these steps:

-   Download the `co2_monthly_maunaloa.txt` from [here](https://github.com/geco-bern/agds/blob/main/data/co2_monthly_maunaloa.txt) and look at the data.
-   Make a simple figure showing the change of average CO$_2$ over time.
-   Do you need to do any data wrangling before continuing?
-   Write a function that computes a 12-month running mean of the CO$_2$ time series.
-   Make nice "publishable" figure that shows the measured and trended timeseries of CO$_2$.
