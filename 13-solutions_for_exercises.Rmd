...
```{r message=FALSE}
# a.
dplyr::starwars %>% filter(skin_color == "pale", homeworld == "Naboo" | homeworld == "Ryloth") %>% nrow      
# b.
...
hhdf |>
  ...
  dplyr::group_by(hod) |>
  nest() |>
  mutate(QC_meas = purrr::map_dbl(data, ~. |> dplyr::filter(NEE_VUT_REF_QC == 0) |> nrow()) / nrows_hhdf,
         QC_fill = purrr::map_dbl(data, ~. |> dplyr::filter(NEE_VUT_REF_QC != 0) |> nrow()) / nrows_hhdf) |>
