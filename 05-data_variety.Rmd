...
We use the `file.path()` function to combine the path (`tempdir()`) with the filename (`your_file.csv`). Using `file.path()` is good practice as directory structures are denoted differently between operating systems e.g., using a backslash (`\`) on Windows vs. a slash (`/`) on Unix based systems (Linux/macOS). The `file.path()` function ensures that the correct directory separator is used.
...
In the case of binary files it will not be possible to read the meta-data as plain text. In this case, specific commands can be used to read the meta-data included in a file. The example below shows how you would list the meta-data of a GeoTiff file using the bash.
...
Meta-data of data read into R can be accessed by plotting the object itself, i.e. calling the object on the command line. If it is a simple table the first lines of the table will be shown. When it is a more complex object the meta-data will be output as a formatted statement. You can also use the `str` or `summary` commands to summarize data and meta-data. For in depth discussions on meta-data and data wrangling look at Chapter \@ref(datawrangling).
...
### Example environmental data repositories

Below you find a list of common environmental data repositories which provide manual or automated download access to their data. Note that this list are some example and is far from extensive.
...
``` r
https://raw.githubusercontent.com/geco-bern/agds/main/data/some_data.nc
```
...
``` r
https://raw.githubusercontent.com/geco-bern/agds/main/data/some_data.tiff
```

1.  Does this data seem familiar, and how can you tell? What are your conclusions?
