...
The set of objects (variables, data frames, etc.) defined during an R session are referred to as the *environment*. You can view the objects in RStudio in the **environment** panel, grouped as Data, Values and Functions (functions that you write, which we cover in Tutorial 2).
...
When opened in RStudio, you can knit an RMarkdown document by clicking the **Knit** button at the top of the source panel. To run the chunks of code in an RMarkdown file, you can click on the **Run** button also on the source panel and select an option from the drop-down menu. For example, **Run All** runs all the chunks in the document. Alternatively, individual chunks can be executed by clicking the green right-pointing triangle in the upper right corner of each chunk.
