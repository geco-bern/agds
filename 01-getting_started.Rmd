...
use_pkgs <- c("dplyr", "tidyr", "readr", "lubridate", "stringr", "purrr",
              ...
              "rpart.plot", "ranger", "sessioninfo", "here")
...
- Open RStudio. Create a new R project from a GitHub repository. Paste the repository's url, git it a name (by default the same as the repository) and select in which folder you want to keep it locally (for example the base directory `~`). This will clone all the files from GitHub onto the project directory in your computer.
- With this, you're set up. You should be able to see the README.md file in your file browser. To test things out, open it and write something, like a description of the repository (it's a markdown file!). 
- Now those changes are only local. To keep track of changes, go to the **Git** panel in RStudio. Tick the files you want to update and click **Commit**, write a short description of the changes you've made (for example, "description project") and *commit*. And to upload changes to GitHub, click **Push**. 
- You'll see a detailed explanation of the *git* workflows in Chapter \@ref(codemgmt). For now, keep adding the report content to this R project and regularly commit your changes to keep track of the versions of your work.
