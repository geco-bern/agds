...
Furthermore, they can be treated as a *matrix*. Note that the first index refers to rows and the second to columns. For example:
...
There are many more things you can do with data frames. Since they are central to analyzing data with R, we go into more detail in [Chapter 2](#programming_primers) and have dedicated all of [Chapter 3](#data_wrangling) to teach you how to work with data frames in a tidy way.
...
Functions are also R objects and have class `"function"`. They are covered in more detail in [Chapter 2](#programming_primers).
...
Note that, to be able to run the code above, the file `my_r_script.R` must be in your current working directory, which you can determine by `getwd()`.
...
A script should start with a brief and concise statment of what it does, followed by statements to load libraries and source functions that are saved in separate `.R` files. Then, start with statements that implement the purpose of the respective script. 

Statements, or groups of statements, should be preceeded by comments. Comments are all that appears to the right of a `#` and are code parts that are not interpreted by R and not executed. Adding comments in the code helps you and others to read your code, understand what it does, modify it, and resolve errors (bugs).
...
A script script could look like this:

```{r eval=FALSE, message=F}
#---------------------------------------
# Create a vector with random values
# Save as file "data/vec_randomvalues.rds"
#---------------------------------------
library(tidyverse)

# Generate data----
# Random 100 values are from a distribution 
# with mean 20 and standard deviation 5
vec <- rnorm(100, mean = 20, sd = 5)

# Write to file----
filn <- "data/vec_randomvalues.rds"
...
*Git* is a version control system (covered in detail in [Tutorial 7](#codemgmt)) which is integrated in RStudio. Code repositories can then be hosted in the cloud, for example in [GitHub](https://github.com/). To use it later in this course (and throughout your career), let's set up these tools:
