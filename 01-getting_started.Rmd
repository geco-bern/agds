...
After closing an existing R session (e.g., after quitting RStudio), the environment defined by the used during that session will not be saved automatically and will be lost. To save your environment, go to the **Session** menu and select **Save Workspace As...**. This will save all your objects in a `.RData` file in your *working directory*. The working directory is the default location to which R writes to and reads files from. You can check what your current working directory is running `getwd()` in the command line.
