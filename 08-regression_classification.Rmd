...
- **Pearson's $r^2$**: The linear correlation between two variables (here $Y$ and $\hat{Y}$) is measured by the *Pearson's correlation coefficient* $r$. Its square is closely related to the coefficient of determination $R^2$.
$$
r = \frac{\sum_i (Y_i - \bar{Y}) (\hat{Y_i} - \hat{\bar{Y}}) }{\sqrt{ \sum_i(Y_i-\bar{Y})^2}\sqrt{ \sum_i(\hat{Y_i}-\bar{\hat{Y}})^2 } }
$$
...
compute_regr_metrics <- function(linmod){
  c(mse = mean(linmod$residuals^2),
    ...
    R2 = 1 - sum(linmod$residuals^2)/
      sum((linmod$model$GPP_NT_VUT_REF- mean(linmod$model$GPP_NT_VUT_REF))^2),
    R2_adj = summary(linmod)$adj.r.squared,
    # r2 = summary(linmod)$r.squared
    cor(linmod$fitted, linmod$model$GPP_NT_VUT_REF)^2 # alternative for r2
