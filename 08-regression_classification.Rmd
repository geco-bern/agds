...
-   [Linear
    regression](https://github.com/stineb/esds_book-gitlab/blob/e27fbc390fa302bc99be3d1796c4e75dc6ee1c41/06_supervised_ml_1.Rmd#L84)
-   Logistic regression
-   [Regression
    metrics](https://github.com/stineb/esds_book-gitlab/blob/e27fbc390fa302bc99be3d1796c4e75dc6ee1c41/07_supervised_ml_2.Rmd#L236)
-   [Classification
    metrics](https://github.com/stineb/esds_book-gitlab/blob/e27fbc390fa302bc99be3d1796c4e75dc6ee1c41/07_supervised_ml_2.Rmd#L345),
    [another
    link](https://bookdown.org/max/FES/measuring-performance.html#class-metrics)
-   Comparing models (AIC, ...)
-   Detecting outliers: identification via distributions [hist(),
    boxplot(), qqnorm()], multivariate (Cook's Distance to get
    influential values)
-   Feature selection, [stepwise
    regression](https://github.com/stineb/esds_book-gitlab/blob/master/08_variable_selection.Rmd),
    multi-colinearity
...
predictor adds the most additional information at each step. XXX
exercises XXX.
...
loss function, see Chapter \@ref(supervised_ml_ii)), and inform model
...
the scatterplots below and how different aspects of the model-data
agreement are measured by different metrics. Definitions of the metrics displayed and other metrics are given below.

```{r correlation_plots, out.height='500%', warning=FALSE, message=FALSE, echo=FALSE}
analyse_modobs <- function(df){
  ...
  subtitle <- bquote( 
    italic(R)^2 == .(df_metrics |> dplyr::filter(.metric == "rsq") |> pull(.estimate_form)) ~~
    RMSE == .(df_metrics |> dplyr::filter(.metric == "rmse") |> pull(.estimate_form)) ~~
    Bias == .(df_metrics |> dplyr::filter(.metric == "bias") |> pull(.estimate_form)) ~~
    Slope == .(df_metrics|> dplyr::filter(.metric == "slope")|> pull(.estimate_form))
    )
...
gg3 <- tibble(x = rnorm(100)) |> 
  mutate(y = x) |> 
  analyse_modobs() +
  labs(title = bquote("Perfectly correlated, zero error:" ~ y == x)) +
  ylim(-3,3) + xlim(-3, 3) +
  theme(plot.title = element_text(size = 10),
        plot.subtitle = element_text(size = 8))
...
gg4 <- tibble(x = rnorm(100)) |> 
  mutate(y = 0.6 * x) |> 
  analyse_modobs() +
  labs(title = bquote("Perfectly correlated, scaling error:" ~ y == ~ .(0.6) ~ x)) +
  ylim(-3,3) + xlim(-3, 3) +
  theme(plot.title = element_text(size = 10),
        plot.subtitle = element_text(size = 8))
...
gg5 <- tibble(x = rnorm(100)) |> 
  mutate(y = x + 1) |>
  analyse_modobs() +
  labs(title = bquote("Perfectly correlated, constant error:" ~ y == x + 1)) +
  ylim(-3,3) + xlim(-3, 3) +
  theme(plot.title = element_text(size = 10),
        plot.subtitle = element_text(size = 8))
...
gg6 <- tibble(x = rnorm(100)) |> 
  ...
  analyse_modobs() +
  labs(title = bquote("Correlated:" ~ y == x + epsilon[0])) +
  ylim(-3,3) + xlim(-3, 3) +
  theme(plot.title = element_text(size = 10),
        plot.subtitle = element_text(size = 8))
...
gg7 <- tibble(x = rnorm(100)) |> 
  ...
  analyse_modobs() +
  labs(title = bquote("Better correlated:" ~ y == x + ~ .(0.7) ~ epsilon[0])) +
...
9](#supervised_ml_i), during model training when used as a loss
...
also called the *coefficient of determination*, describes the
...
*goodness of fit*. It is traditionally defined as:
$$
R^2 = 1 - \frac{\sum_i (\hat{Y}_i - Y_i)^2}{\sum_i (Y_i - \bar{Y})^2}\\
$$
A perfect fit is quantified by $R^2 = 1$. There is not generally
valid threshold of $R^2$ for a model to be considered "good". It
depends on the application and the nature of the data and the
data-generating process. Note that above equation can also be
written as $R^2 = 1 - \text{MSE}/var(Y)$.

**Pearson's-r**

The linear association between to variables (here $y$ and $\hat{y}$) is measured by the *Pearson's correlation coefficient* $r$. 
$$
...
related to the coefficient of determination-$R^2$ and in common
cases of prediction-observation comparisons almost identical. For a
linear regression, they are identical (see proof
[here](https://statproofbook.github.io/P/slr-rsq)). 
...
The `yardstick::rsq_trad()` returns the coefficient of determination as
defined above and is not equal to the values above.

```{r}
1 - sum((df$x - df$y)^2) / sum((df$y - mean(df$y))^2)
yardstick::rsq_trad(df, y, x) %>% pull(.estimate)
```
...
negative ($r$ can attain positive or negative values). The
coefficient of determination can return negative values when
comparing observed and predicted values for uninformative estimates
and is thus not actually bound between 0 and 1. Therefore, the
definition of the coefficient of determination $R^2$ as given above
is to be avoided. For subsequent chapters, we will use '$R^2$' to refer to the square of the Pearson's-*r*.
...
[9](#supervised_ml_i) and [10](#supervised_ml_ii), can also be used for
