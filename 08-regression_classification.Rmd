...
- Explain how the metrics from above can be used to find the best model. Increase complexity slowly, etc.
...
A brute force approach to variable selection would be: Fit a linear regression for each combination of available predictors, calculate a metric (e.g. AIC) and choose the best one (lowest AIC). The problem is, if you have 8 predictors, you would fit 40320 different regression models.

Instead, take a hierarchical greedy approach, starting with an empty model (just an intercept) and adding one variable at a time. At each step, you 
...
gg12 <- data.frame(x = c(x, 3.9), y = c(y, 3.9)) |>
    ggplot(aes(x = x, y = y)) +
  geom_point(size = 0.75) +
  geom_smooth(method = "lm", color = "red") +
  xlim(-3, 4) +
...
In addition, R provides some useful plots from the fitted regression objects. 
 identification via distributions [hist(), boxplot(), qqnorm()], multivariate (Cook's Distance to get influential values)
