...
```{r eval=FALSE}
half_hourly_fluxes <- mutate(half_hourly_fluxes, TIMESTAMP_START = ymd_hm(TIMESTAMP_START))
```
...
```{r}
plot(half_hourly_fluxes[1:(2*24),]$TIMESTAMP_START, half_hourly_fluxes[1:(2*24),]$SW_IN_F, type = "l")
```
...
```{r}
plot(half_hourly_fluxes[1:(365*2*24),]$TIMESTAMP_START, half_hourly_fluxes[1:(365*2*24),]$SW_IN_F, type = "l")
```
...
# save for later use
write_rds(daily_fluxes_allsites_nested_joined, file = "data/daily_fluxes_allsites_nested_joined.rds")
```
...
-   Reuse the code in the tutorial to aggregate the `half_hourly_fluxes` dataset to the daily scale and calculate the following metrics for `VPD_F` using R functions: maximum value, minimum value, median, and standard deviation.
