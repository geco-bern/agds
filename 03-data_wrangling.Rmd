...
```{r eval=FALSE}
hhdf$TIMESTAMP_START <- lubridate::ymd_hm(hhdf$TIMESTAMP_START)
```
...
```{r eval=FALSE}
hhdf <- dplyr::mutate(hhdf, TIMESTAMP_START = lubridate::ymd_hm(TIMESTAMP_START))
```
...
```{r, eval=FALSE}
hhdf <- hhdf |> 
  dplyr::mutate(TIMESTAMP_START = ymd_hm(TIMESTAMP_START))
```
...
> Note: If you have an older Macbook that can't operate the latest Operating System version, you may not be able to update R. In this case, just use the `magrittr` pipe throughout.
...
```{r}
hhdf |>
  dplyr::mutate(year = lubridate::year(TIMESTAMP_START),
         month = lubridate::month(TIMESTAMP_START)) |>
  dplyr::select(TIMESTAMP_START, TIMESTAMP_END, year, month)  # for displaying
```
...
df1 <- tibble::tibble(co2_concentration = as.vector(co2), 
       year_dec = time(co2)
       ) |> 
...
df2 <- df1 |> 
  tidyr::pivot_wider(names_from = month, values_from = co2_concentration)

df3 <- df1 |> 
  # dplyr::mutate(year = as.character(year)) |> 
  tidyr::pivot_wider(names_from = year, values_from = co2_concentration)
```
...
```{r}
ddf <- hhdf |>  
  dplyr::mutate(date = as_date(TIMESTAMP_START)) |>  # converts the ymd_hm-formatted date-time object to a date-only object (ymd)
  dplyr::group_by(date) |> 
  dplyr::summarise(SW_IN_F = mean(SW_IN_F))
```
...
```{r}
ddf <- hhdf |> 
    dplyr::mutate(date = as_date(TIMESTAMP_START)) |>   # converts time object to a date object
    dplyr::group_by(date) |> 
    ...
    dplyr::mutate(f_measured = n_measured / n_datapoints) # calculate the fraction of measured values over total observations
...
```{r message=FALSE}
hhdf |>  
  dplyr::mutate(date = as_date(TIMESTAMP_START)) |>
  group_by(date) |> 
  nest()
```
...
```{r}
hhdf |> 
  dplyr::select(TIMESTAMP_START, starts_with("SWC_F_MDS_")) |> 
  head()
```
...
```{r eval=FALSE}
hhdf |> 
    dplyr::mutate(GPP_NT_VUT_REF = ifelse(NEE_VUT_REF_QC %in% c(0,1), GPP_NT_VUT_REF, NA))
```
...
```{r}
df6 |>
  dplyr::slice(sample(1:n(), replace = FALSE)) |>  # re-shuffling rows
...
```{r}
visdat::vis_miss(
  ddf_allsites |> dplyr::slice(1:10000),
  cluster = FALSE,
  warn_large_data = FALSE
  )
```
...
ddf_allsites <- ddf_allsites |>
  dplyr::mutate(siteid = str_sub(siteid, start = 12, end = 17))
...
ddf_allsites_nested_joined <- siteinfo_fluxnet2015 |>
  dplyr::rename(siteid = sitename) |>
  dplyr::right_join(dplyr::select(ddf_allsites_nested, -linmod, -summ, -rsq),
             by = "siteid")

ddf_allsites_joined <- siteinfo_fluxnet2015 |>
  dplyr::rename(siteid = sitename) |>
  dplyr::right_join(ddf_allsites,
             by = "siteid")
...
a.  How many pale characters come from the planets Ryloth and Naboo?
b.  Who is the oldest of the tallest 5 characters?
c.  What is the name of the smallest character and their starship in "Return of the Jedi"

-   Reuse the code in the tutorial to aggregate the `hhdf` dataset to the daily scale and calculate the following metrics for `VPD_F` using R functions: maximum value, minimum value, median, and standard deviation.
-   The uncleaned dataset `FLX_CH-Lae_FLUXNET2015_FULLSET_HH_2004-2006.csv` holds half-hourly data that is sometimes not of high quality. Split your dataset into two - one with actually measured data (`NEE_VUT_REF_QC == 0`) and one with only gap-filled data. Compare the means of `GPP_NT_VUT_REF` of all three datasets and interpret your finding. Can you find a connect between the timepoint of an entry and its quality? What does the temporal pattern imply for the mean values that you calculated?

<!-- -   The elevation data in `siteinfo_fluxnet2015` is a continuous numerical variable. Create one ore more variables that turns elevation into a categorical variable with three levels: < 0, 0-1000, 1001-2000, > 2000. Can you find a elegant way to add that this data are in units of `m`? -->
