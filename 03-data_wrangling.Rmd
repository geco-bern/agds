...
Your task is to manually clean the data in the tab "Database S1" and save it as a CSV file that can be read into R.
...
A log-response ratio (treatment effect) can be calculated as:
$$
R = \ln \left( \frac{x_\text{elevated}}{x_\text{ambient}} \right)
$$

Perform the following steps:

- Aggregate data per experiment and calculate the log-response ratio within each experiment.
...
Document and describe your workflow step-by-step in an RMarkdown file (cleaning, reading, wrangling, aggregating). Use R code chunks to run your code and present your results as tables using the `knitr::kable()` function. Knit your RMarkdown file to HTML and ensure reproducibility of your code (more on this in later Chapters).
