...
R](./figures/supervised_ml.png){width="13cm"}
...
regression model for `GPP_NT_VUT_REF` with three predictors `SW_IN_F`,
...
Finally we can actually transform the data. That is, "juice" the
prepared recipe.
...
Note, if we are to apply the prepared recipe to new data, we'll have to
`bake()` it.
