...
In supervised machine learning, we use a set of *predictors* $X$ (also known as *features*, or *labels*, or *independent variables*) and observed values of a target variable $Y$ that are recorded in parallel, to find a model $f(X) = \hat{Y}$ that yields a good match between $Y$ and $\hat{Y}$ and that can be used for reliably predicting $Y$ for new ("unseen") data points $X_\text{new}$ - data that has not been used during model fitting/training. The hat on $\hat{Y}$ denotes an estimate. Some algorithms can even handle predictions of multiple target variables simultaneously (e.g., neural networks).
...
```{r mlingredients, echo = FALSE, fig.cap="Supervised machine learning ingredients, adopted from Chollet & Allaire (2018) Deep Learning with R"}
...
We can use the same fitted models on data that was *not* used for model fitting - the *test data*. This is what's done below. Again, the same true underlying relationship is used, but we sample a new set of data points $X$ and add a new sample of errors on top of the true relationship.
...
daily_fluxes_train_plot <- daily_fluxes_train |> 
  dplyr::select(one_of(c("SW_IN_F", "VPD_F", "TA_F"))) |> 
  tidyr::pivot_longer(cols = c(SW_IN_F, VPD_F, TA_F), names_to = "var", values_to = "val")

# plot density
plot_1 <- ggplot(data = daily_fluxes_train_plot, aes(val, ..density..)) +
  geom_density() +
  facet_wrap(~var)

# plot density by var
plot_2 <- ggplot(data = daily_fluxes_train_plot, aes(val, ..density..)) +
  geom_density() +
  facet_wrap(~var)
...
1.  The last figures of this tutorial show the training of testing of a linear model and a KNN model. Compare the four figures and answer to following questions:
    a.  How does the point cloud differ between the two training set plots? Why does the $R^2$ value increase from LM to KNN? Use your knowledge on the fundamental difference between the two fitted models!
    b.  How does the performance of both models (LM and KNN) change when predicting onto the test set? Why do they change in that manner?
    c.  The test set plots for both models show a tendency to have more points on the right side of the dotted one-to-one line. What does that tell you about the performance of the model? How do you interpret this model performance in an ecophysiological manner? Note that we are comparing predicted (y-axis) against observed (x-axis) values of gross primary production.

The last figures of this tutorial show the training of testing of a linear model and a KNN model. Compare the four figures and answer to following questions:

1.  How does the point cloud differ between the two training set plots? Why does the $R^2$ value increase from LM to KNN? Use your knowledge on the fundamental difference between the two fitted models!

2.  How does the performance of both models (LM and KNN) change when predicting onto the test set? Why do they change in that manner?

3.  The test set plots for both models show a tendency to have more points on the right side of the dotted one-to-one line. What does that tell you about the performance of the model? How do you interpret this model performance in an ecophysiological manner? Note that we are comparing predicted (y-axis) against observed (x-axis) values of gross primary production.

### The role of picking a hyperparameter {-}

Let's look at the role of $k$ in a KNN. Answer the following questions:

1.  Write a hypothesis based on the following questions: What do you expect, how do the model metrics of both sets, the training set **and** the test set, change if you increase or decrease $k$? Connect this question to what you learned about the bias-variance trade-off!

2.  Put your hypothesis to the test! Write a code script that repeats fitting different values for $k$ as done in the tutorial. Use a simple bivariate model, regressing temperature against the day-of-year in the `daily_fluxes` dataset (`DOY ~ TA_F`). **Visualise** your results in an intuitive plot that is understandable without additional explanation.

3.  Compare your results against your hypothesis from 2a. Is the result as you expected? If your hypothesis was right, can you explain further, why your hypothesis was correct, using the results. If your hypothesis was wrong, can you explain why? What surprised you, what did you not expect to see?
